...
export default function AddTrip() {
				...
				try {
					...
					if (data.location && data.location.waypointsFeatures) {
						const waypointsFeatures = JSON.parse(
							data.location.waypointsFeatures
						)
	...
	useEffect(() => {
		if (direction && mapData.location) {
			const waypointsFeatures = JSON.parse(
				mapData.location.waypointsFeatures || "[]"
			)
			const snappointsFeatures = JSON.parse(
				mapData.location.snappointsFeatures || "[]"
			)
			const routelinesFeatures = JSON.parse(
				mapData.location.routelinesFeatures || "[]"
			)

			direction.setWaypointsFeatures(waypointsFeatures)
			direction.setSnappointsFeatures(snappointsFeatures)
			direction.setRoutelinesFeatures(routelinesFeatures)
		}
	}, [direction, mapData])
		...
		<>
			...
			<div className="w-full h-screen bg-stone-900 overflow-x-hidden pb-4">
				...
				<div className="mt-2 max-lg:text-center">
					<span className="font-kanit text-4xl text-white lg:ml-20 px-4 mt-4 ">
						Edit Trip
					</span>
				</div>
					...
					<div className="w-full lg:w-1/2">
						...
						>
							...
							<div className="w-full md:px-3">
								<p className="block uppercase tracking-wide text-neutral-100 text-sm font-kanit mb-4">
									Add Image
								</p>
								...
								<p className="font-kanit text-neutral-500 font-light text-sm">
									Note: Original images will be replaced
								</p>
							</div>
							{error && (
